...
| `Cpp2IL.Core/IlGenerator.cs` | Also: the call's result is stored only when `ResultFitsItsDestination` (fork) agrees. Inside a shared generic the destination is typed by the stand-in `Dictionary<int, object>` while the call returns the real instantiation, and storing across that reads back as a cast C# refuses - which cost `BoardController::ComputeHighlights` all fifty of its branches before the guard existed. |
