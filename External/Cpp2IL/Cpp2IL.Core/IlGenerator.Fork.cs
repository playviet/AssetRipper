...
public static partial class IlGenerator
{
    ...
    /// </remarks>
    /// <summary>
    /// Writes the <c>catch</c> clauses <see cref="Analysis.CatchClauses"/> recovered as real CIL exception
    /// handlers, so that their bodies are reachable and the decompiler writes them out.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The layout is already true when this runs: <see cref="LayoutOrder"/> puts every recovered handler last,
    /// in clause order, so each is one contiguous run and the runs are consecutive. A clause's <c>try</c> is
    /// the guarded block's own run, which need not be adjacent to its handler.
    /// </para>
    /// <para>
    /// Three things CIL asks for that the ISIL does not know about. A handler is entered with the exception on
    /// the stack, so each opens with a <c>pop</c> - none of these names it, because a C++ landing pad reads it
    /// out of the runtime rather than off the stack. Control may not <b>fall</b> into a handler. And a
    /// protected region may not be fallen out of, returned out of or branched out of: <c>leave</c> is the only
    /// way, so a <c>ret</c> becomes a store to one local and a <c>leave</c> to the one epilogue, a <c>br</c>
    /// becomes a <c>leave</c>, and a two-way exit becomes an inverted test jumping over one.
    /// </para>
    /// <para>
    /// Every boundary is looked up by identity, and if any one clause cannot be placed the method keeps none -
    /// a half-written set of ranges is worse than no ranges at all.
    /// </para>
    /// </remarks>
        ...
        Dictionary<Instruction, List<CilInstruction>> instructionMap)
    {
        ...
        foreach (var placement in placed)
        {
            ...
            switch (last.OpCode.Code)
            {
                ...
                //Both arms leave, so it is a `leave` each and the inverted test choosing between them:
                //
                //    brtrue X            ->    brfalse L
                //                              leave   X
                //                          L:  leave   N     (where it used to fall)
                case CilCode.Brtrue or CilCode.Brtrue_S or CilCode.Brfalse or CilCode.Brfalse_S:
                    if (last.Operand is not CilInstructionLabel { Instruction: { } named }
                        || tryLast + 1 >= At(instructions, placed[0].HandlerFirst))
                    {
                    ...
                    last.OpCode = last.OpCode.Code is CilCode.Brtrue or CilCode.Brtrue_S
                        ? CilOpCodes.Brfalse
                        : CilOpCodes.Brtrue;
                    last.Operand = new CilInstructionLabel(otherwise);

                    placement.TryLast = otherwise;
                    break;
                ...
                default:
                    ...
                    var fallout = new CilInstruction(CilOpCodes.Leave, new CilInstructionLabel(instructions[tryLast + 1]));
                    instructions.Insert(tryLast + 1, fallout);
                    placement.TryLast = fallout;
                    break;
            }
        }
        ...
        //Backwards, so the insertions do not move the indices still to be visited.
        ...
        //Nothing may fall into a handler. Backwards again, so an insertion before one cannot move another
        ...
        //protected region. Only branches out of the handlers' own territory: one inside is an ordinary jump,
        //and the two are told apart by position, which is exact because each handler is a contiguous run and
        ...
        for (var i = territory; i < beyond; i++)
        {
            ...
            //Only the named arm leaves; the fall-through stays inside and carries on.
        ...
        //And the ranges last, when nothing else is going to move. A handler ends where the next begins, and
        //the final one at the epilogue.
