    ...
    private static void ReplaceLocalsUntilReassignment(Block block, int startIndex, LocalVariable local, object replacement, bool stopAtJoins)
    {
        ...
        void ProcessBlock(Block currentBlock, int index)
        {
            ...
            for (var i = index; i < currentBlock.Instructions.Count; i++)
            {
                ...
                // Stop when what is being carried forward stops being true. A local holds the value it was
                // given; a field or a memory location holds whatever was last written there, so carrying one
                // past a write to it - or past a call, which can write anything - turns a value that was read
                // once into one that is read again later, and the two are not the same value.
                if (Invalidates(instruction, replacement))
                    return;
            ...
            foreach (var successor in currentBlock.Successors)
            {
                ...
                // A value read out of memory has to stop there too, whatever its local's definitions look
                // like: the substitution is made in the instruction, which every path into the join shares,
                // and only one of those paths has to write the field for the value to differ along it.
                if ((stopAtJoins || IsReadFromMemory(replacement)) && successor.Predecessors.Count > 1)
                    continue;
