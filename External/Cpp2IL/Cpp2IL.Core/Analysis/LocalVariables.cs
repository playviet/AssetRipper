...
public static partial class LocalVariables
{
    ...
    private static void SeedNewobjResults(MethodAnalysisContext method)
    {
        ...
        foreach (var instruction in method.ControlFlowGraph.Instructions)
        {
            ...
            if (instruction.Operands[0] is LocalVariable destination && InstantiatedType(instruction.Operands[1], definitions) is { } type)
                destination.Type = type;
        }
    }

    /// <summary>
    /// The type an <c>object_new</c> allocated, from the operand holding its runtime class.
    /// </summary>
    /// <remarks>
    /// The class is often not in the local the call reads: the argument register is loaded from the one the metadata
    /// global was loaded into, and each copy is its own value in single assignment form. Only the first of them
    /// carries the type, so the copies have to be followed back to it.
    /// </remarks>
    private static TypeAnalysisContext? InstantiatedType(object classOperand, Dictionary<LocalVariable, Instruction>? definitions = null)
    {
        ...
        while (true)
        {
            switch (classOperand)
            {
                case LocalVariable { Type: RuntimeClassTypeAnalysisContext { RepresentedType: var represented } }:
                    return represented;
                case RuntimeClassTypeAnalysisContext { RepresentedType: var represented }:
                    return represented;
                case TypeAnalysisContext type:
                    return type;
                case LocalVariable local when definitions != null && visited.Add(local)
                    && definitions.TryGetValue(local, out var definition)
                    && definition.OpCode == OpCode.Move && definition.Operands.Count > 1:
                    classOperand = definition.Operands[1];
                    continue;
                default:
                    return null;
            }
    ...
    {
        var changed = false;
        var addresses = AddressesUsedAsBases(method);
        ...
        foreach (var instruction in method.ControlFlowGraph!.Instructions)
        {
            switch (instruction.OpCode)
            {
                ...
                case OpCode.Phi:
                    ...
                    break;
                case OpCode.Select:
                    changed |= PropagateSelect(instruction);
                    break;
                case >= OpCode.CheckEqual and <= OpCode.CheckLessOrEqual:
                    changed |= PropagateComparison(instruction);
                    break;
                case OpCode.Add:
                ...
                case OpCode.ShiftRight:
                    changed |= PropagateArithmetic(method, instruction, addresses);
                    break;
            }
        }
    ...
    private static bool PropagateFromCallParameters(MethodAnalysisContext method)
    {
        ...
        foreach (var instruction in method.ControlFlowGraph!.Instructions)
        {
            ...
            {
                var produced = calledMethod.Name is ".ctor" or ".cctor" ? calledMethod.DeclaringType : calledMethod.ReturnType;

                changed |= SetTypeIfUnknown(returnValue, produced);
                changed |= SharpenFromReturn(returnValue, produced);
            }
            ...
            for (var i = paramOffset; i < instruction.Operands.Count; i++)
            {
                ...
                //A struct that travels in several registers is only named by the first of them, so what that
                //register holds is one field of it rather than the whole thing. Taking the parameter's type
                //from here would call a float a Vector3, and every use of it after that reads as one.
