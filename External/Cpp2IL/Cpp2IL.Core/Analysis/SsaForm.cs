...
public partial class SsaForm
{
    ...
    private void InsertPhiFunctions(ISILControlFlowGraph graph, DominatorInfo dominance)
    {
        ...
        var liveIn = LiveInRegisters(graph);
        ...
        foreach (var entry in defSites)
        {
            ...
            while (workList.Count > 0)
            {
                ...
                foreach (var frontierBlock in frontier)
                {
                    ...
                    // Nothing there can read the register, so there is nothing for a phi to decide - and
                    // merging a finished value into a live one loses what the live one is.
