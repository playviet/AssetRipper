...
public class IlGeneratorTests
{
    ...
    public void StaticCall_DoesNotLoadMethodInfoOperand()
    {
        ...
        //A value used once travels on the stack rather than through a local, so what the two parameters are
        //loaded from is whatever produced them - here the two literals. The count is what matters: a third
        //push would be the trailing operand being passed as an argument.
        Assert.That(il.Count(i => i.OpCode == CilOpCodes.Ldloc || i.OpCode == CilOpCodes.Ldc_I4), Is.EqualTo(2),
            "expected exactly two values to be pushed for the two parameters of the target method");
    }
