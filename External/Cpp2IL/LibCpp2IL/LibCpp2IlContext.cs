...
public sealed class LibCpp2IlContext
{
    ...
    public MetadataUsage? CheckForPost27GlobalAt(ulong address)
    {
        ...
        //A shared object cannot address its usage globals directly: they are extern with default visibility, so they
        //are preemptible and the compiler has to reach them through the global offset table. On such a binary the word
        //at this address is a pointer to the usage rather than the usage itself, so follow it once. An executable
        //reaches them directly and never gets here.
        return CheckForPost27GlobalBehind(encoded);
    }

    /// <summary>
    /// The usage a global offset table slot points at, if the address is one and it points at a usage.
    /// </summary>
    private MetadataUsage? CheckForPost27GlobalBehind(ulong address)
    {
        if (address == 0 || !Binary.TryMapVirtualAddressToRaw(address, out var raw) || raw >= Binary.RawLength)
            return null;

        var metadataUsage = MetadataUsage.DecodeMetadataUsage(Binary.ReadPointerAtVirtualAddress(address), address, this);

        return metadataUsage?.IsValid == true ? metadataUsage : null;
    }
