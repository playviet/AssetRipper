...
public class Il2CppMethodDefinition : ReadableClass
{
    ...
    /// <summary>
    /// This method's own runtime generic context, which a generic method reads through
    /// <c>MethodInfo::rgctx_data</c> for the types and methods its body needs but its declaring type
    /// does not supply. The ranges are keyed by metadata token and hold methods alongside types, so the
    /// same lookup a type does finds a method's entries.
    /// </summary>
    public Il2CppRGCTXDefinition[] RgctXs
    {
        get
        {
            ...
            if (MetadataVersion >= 108)
            {
                ...
                return range == null ? [] : metadata.RgctxValues!.SubArray(range.start, range.length);
            }

            if (declaringType.CodeGenModule is not { } cgm)
                return [];

            var rangePair = cgm.RGCTXRanges.FirstOrDefault(r => r.token == token);

            return rangePair == null ? [] : OwningContext.Binary.GetRgctxDataForPair(cgm, rangePair);
        }
    }
