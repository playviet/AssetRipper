            ...
            foreach (USILInstruction instruction in instructions)
            {
                foreach (USILOperand operand in instruction.srcOperands)
                {
                    if (operand.operandType == USILOperandType.SamplerRegister)
                    {
                        ...
                        if (texParam == null)
                        {
                            ...
                            if (texParam == null)
                            {
                                ...
                                Logger.Warning(LogCategory.Export, $"Could not find texture parameter for sampler {operand}");
                                continue;
                            }
                        }
                    ...
                    else if (operand.operandType == USILOperandType.ResourceRegister)
                    {
                        ...
                        if (texParam == null)
                        {
                            Logger.Warning(LogCategory.Export, $"Could not find texture parameter for resource {operand}");
                            continue;
                        }
